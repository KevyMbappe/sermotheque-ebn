...
3. **Finish the web POC** (#54, `apps/web-poc/` — resume notes: `docs/plans/WEB-POC-STATUS.md`, which lists the exact remaining steps). It is a **demo for the elders + de-risking of the WordPress library**, not a replacement for it. Left to do, in order: verify the **sermon page on screen** (player seek + synced transcript — the one thing that matters, never yet seen working), then a **YouTube** sermon (IFrame path never executed), then `npm run build && preview` (hash routing untested in build). Deploy question open: Pages needs a paid plan on a private repo.
