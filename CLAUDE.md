...
6. **RUN the full ASR enrichment pass** — runner ready (`pipeline/run_enrichment.py`, resumable + logfile + cost), default-speaker rule (#45) + voiceprint capture (#46) in. Run: `./.venv/bin/python pipeline/run_enrichment.py --source soundcloud`. **Measured cost $0.0705/sermon ⇒ ~$17 for the 239 SC / ~$36 for the full 517** (Haiku ≈ ⅓). *Top up credit, or run the SC spine on Sonnet + the YT-only bulk on Haiku.* Needs `ANTHROPIC_API_KEY`. **Before/alongside the full pass, decide #47(a):** EN sermons (28) — anchor quotes/chapters in source language or accept no deep-links (re-runnable later either way). **YouTube path: add retry/back-off** for the 403 throttling before the YT bulk. *(Text-derived enrichment is re-runnable from committed transcripts without re-ASR; only audio-derived data — transcript/timestamps/voiceprint — is truly one-shot.)*
