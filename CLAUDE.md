...
3. **Finish the web POC** (#54, `apps/web-poc/` — resume notes: `docs/plans/WEB-POC-STATUS.md`). It is a **demo for the elders + de-risking of the WordPress library**, not a replacement for it. Build, hash routing, mobile layout, the sermon page and — confirmed on desktop by the author — **the chapter→player seek** are all verified (M6b). The Pages workflow is written and the repo is public (#55), so **merging to `main` deploys it**. Left: the **YouTube IFrame path** (a distinct code path, never exercised — the dev container blocks YouTube) and an EN sermon.
