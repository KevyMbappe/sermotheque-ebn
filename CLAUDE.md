...
| `pipeline/topics.py` | **Curated topic vocabulary (#57):** 44 categories + aliases taken from the real data; `canonicalize` / `normalize_topics` → the derived `topics_canonical`. `--report` lists what is still unclassified (the curation worklist); `build.py` refreshes `data/catalog/topics.json`. |
...
| `tests/` | Stdlib `unittest` suite (offline). Run: `python3 -m unittest discover -s tests`. Le POC web a la sienne : `cd apps/web-poc && npm test` (`node --test`, sans dépendance). |
| `apps/web-poc/` | **WIP (#54)** — first *consumption* surface: static React 18 + Vite 6 app (elder demo + de-risks the WP library; it does not replace it). `scripts/build-data.mjs` projects `data/catalog/` → `public/data/` before every dev/build (enriched rows + VTT), so the site can't drift from the committed dataset. Pages Home/Browse/Book/Topics/Sermon (no speaker directory — removed on purpose, most speakers are default-rule guesses); VTT-synced transcript, chapter deep-links, **shareable timestamped URLs** (`?t=`), **per-sermon pre-rendered pages** for link previews, and **browse-by-passage** over the OSIS citations (#56) — 54 books vs 22 preached — **browse-by-theme** over the curated vocabulary (#57, 44 themes), weighted full-text search with match snippets, a printable home-group sheet, and generated 1200×630 preview cards. Publishes 131 of the 139 enriched. Mobile layout measured clean 320→768 px; chapter→player seek confirmed on desktop. **LIVE: https://kevymbappe.github.io/sermotheque-ebn/** (auto-redeploys on `data/catalog/**` changes). |
...
- **Topics:** curated vocabulary (**#57 — built: 44 categories, 98% coverage**), derived not stored; labels FR, EN/PT later.
...
- **Topics (2026-08-01, #57):** the free-text labels are **1 086 occurrences / 594 distinct forms, 451 seen once** — unusable as a browse axis. A **44-category curated vocabulary** (`pipeline/topics.py`) derives `topics_canonical`: **98% of occurrences classified**, the last 14 being book names, excluded on purpose. Extending the vocabulary re-labels the whole catalog at **zero API cost**.
...
3. **Polish the web POC — it is LIVE** at **https://kevymbappe.github.io/sermotheque-ebn/** (#54/#55, M6c, 2026-08-01): 131 enriched sermons, player + chapter deep-links + synced transcript, mobile-clean 320→768 px, and it **republishes itself** whenever `data/catalog/**` changes. Its purpose is settled (#58): a **permanent playground** that shows what the catalog makes possible — its design and clever features *are* the product — and an executable roadmap for the future WordPress library, not a replacement for it and not a throwaway. La fiche sermon a été restructurée le 2026-08-02 (M6h) : ordre écoute-d'abord, blocs repliables, bloc lecteur collant. Full-text search over the transcripts landed 2026-08-04 (#59) — a query now returns *moments*, not just pages. Left: confirming the YouTube path against the real API (see 4), an EN sermon, the **8 sermons needing a ~$0.55 re-enrich** to be publishable, and **5 rows with an empty `title`** (upstream parser fix). Resume notes: `docs/plans/WEB-POC-STATUS.md`.
4. **Confirm the YouTube path on a networked machine** — the integration logic is now verified against a faithful stub of the documented API, and three real defects were fixed (#60); the **source badge** and **platform filter** shipped with it. What a stub cannot prove: that YouTube behaves as documented — specifically that `YT.Player` attaches to an existing `youtube-nocookie.com` iframe. Open one of the 12 published videos on any machine with YouTube access and click a chapter.
5. **A real design pass on the POC** — not a CSS cleanup but genuine prototyping (à la Matt Pocock's `/prototype` skill). The current CSS is fluid but authored desktop-out (base styles + one `max-width: 820px` breakpoint), and the visual system grew page by page. **This is not cosmetic work:** per #58, the design *is* the POC's product.
...
**Still-open design questions:** SoundCloud trim (only the sermon, or intro/offering too?), canonical hosting location, dedup edge cases (multi-part, re-uploads). *(« Web POC vs WP import » is settled — see **#58**: the POC is a permanent playground whose purpose is to show what the data makes possible; its statement of purpose is `apps/web-poc/README.md`.)* See docs/SERMOTHEQUE.md §8.
