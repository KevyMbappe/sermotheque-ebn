...
| `apps/web-poc/` | **WIP (#54)** — first *consumption* surface: static React 18 + Vite 6 app (elder demo + de-risks the WP library; it does not replace it). `scripts/build-data.mjs` projects `data/catalog/` → `public/data/` before every dev/build (enriched rows + VTT), so the site can't drift from the committed dataset. Pages Home/Browse/Sermon; VTT-synced transcript + chapter deep-links. Publishes 131 of the 139 enriched. Sermon page not yet verified on screen; not deployed. |
...
- **Catalog state (2026-07-30, counted from `data/`):** 517 rows · **139 transcribed** (`.txt`+`.vtt`, 130 `.raw.txt`) · **217 voiceprints** · **139 enriched** (126 SC / 13 YT) · speakers 100 audio-fingerprint / 45 title / 331 default-rule / 41 blank · languages 491 fr / 26 en · kinds 490 sermon / 12 teaching / 9 teaching-or-qa / 6 qa · 88 tests green.
...
3. **Finish the web POC** (#54, `apps/web-poc/` — resume notes: `docs/plans/WEB-POC-STATUS.md`). It is a **demo for the elders + de-risking of the WordPress library**, not a replacement for it. Build, hash routing, mobile layout and the sermon page's *rendering* are now verified (M6b); the Pages workflow is written and the repo is public (#55), so **merging to `main` deploys it**. **The one thing still unproven is the interaction that justifies the whole pipeline** — clicking a chapter and having the player seek, with the transcript following. It needs a machine with real network access: the dev container blocks SoundCloud/YouTube, so the SDKs never load. Same for the YouTube IFrame path.
...
5. **Close the #47 text-layer findings** when convenient — EN timestamp-anchoring and short-clip fabrication; both fixable from committed transcripts, no re-ASR.
