...
3. **Polish the web POC — it is LIVE** at **https://kevymbappe.github.io/sermotheque-ebn/** (#54/#55, M6c, 2026-08-01): 131 enriched sermons, player + chapter deep-links + synced transcript, mobile-clean 320→768 px, and it **republishes itself** whenever `data/catalog/**` changes. Its purpose is settled (#58): a **permanent playground** that shows what the catalog makes possible — its design and clever features *are* the product — and an executable roadmap for the future WordPress library, not a replacement for it and not a throwaway. La fiche sermon a été restructurée le 2026-08-02 (M6h) : ordre écoute-d'abord, blocs repliables, bloc lecteur collant. Full-text search over the transcripts landed 2026-08-04 (#59) — a query now returns *moments*, not just pages. Left: the **YouTube IFrame path** (see 4), an EN sermon, the **8 sermons needing a ~$0.55 re-enrich** to be publishable, and **5 rows with an empty `title`** (upstream parser fix). Resume notes: `docs/plans/WEB-POC-STATUS.md`.
4. **De-risk the YouTube IFrame path** — a distinct code path from SoundCloud, never exercised (the dev container blocks YouTube). Shipping it also means **a source badge on every card** (YouTube / SoundCloud, in the design system's colours) and probably a **platform filter**: 278 of the 517 sermons are YouTube-only, so the distinction will be visible everywhere once the backfill advances.
