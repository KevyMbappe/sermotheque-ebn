...
**Done:** planning/specs · M1 catalog · M1b series · M2 YT↔SC matching · M2b duration dedup (union 467) · M3 ASR+LLM spike (PASS) · M3b n=8 sample · M4 fold→unified 467 · **M5 enrichment pipeline `build_entry`** · M5b POC (8 real sermons) · M5c cost + Haiku-vs-Sonnet bake-off · M5d timestamp capture · M5e docs restructure · **M5f pipeline hardening (cap 120k, YT date, live tracking + cost, txt+vtt default) + first real YT run** · **M5h matcher hardening (false positives fixed; union 467→517)** · **M5i enrichment writeback (#44)** · **M5j production runner + default-speaker rule (#45)** · **M5k/l audio voiceprint validated + rich schema (#46)** · **M5m voiceprint capture adapter** · **M5n 30-sermon checkpoint (#47, store→39, ~$0.0705/sermon)** · **M5o language-from-audio + retry-on-garbage (#48)** · **M5p–q audio speaker attribution + series priors (#49–#52: all 5 voices, 100 rows audio-fingerprint, LOO 99%)** · **M5s capture/enrich decoupled + rebuild integrity (#52)** · **M5t frozen record contract + Sonnet-only (#53)** · 88 tests · git + GitHub remote.

Catalog is the **unified union of 517 records** (239 SoundCloud + 278 YouTube-only), one canonical schema with `source` + `media`, 26 series. The per-sermon pipeline is **validated end-to-end at batch scale** — a hand-picked **30-sermon checkpoint** (M5n, 2026-06-17) ran clean. **Enrichment persists across rebuilds** via an id-keyed store (`data/catalog/enrichment.json`) + writeback in `build.py` (#44) — **39 rows enriched (~7.5% of 517)**; **voiceprints captured for 46** (`data/catalog/voiceprints.json`, #46). **Measured cost: $0.0705/sermon ⇒ ~$36 for all 517.** The full catalog-wide pass has NOT yet run. Two text-layer findings from the checkpoint (#47), **neither blocking** (both re-runnable from committed transcripts, no re-ASR): EN sermons lose timestamp-anchoring (French anchors vs English VTT); short clips fabricate structure.
...
5. **30-SERMON CHECKPOINT DONE (2026-06-17, M5n / #47)** — hand-picked spread (18 SC / 12 YT, 3 EN, edge cases) → **39 enriched, 46 voiceprints**, **measured $0.0705/sermon ⇒ ~$36/517**. Validated batch robustness (resumability absorbed harness reaping; SC flawless, YT needs retry) + voiceprint capture live. Surfaced two non-blocking text-layer findings (#47). ✓
6. **RUN the full pass** — runner ready (`pipeline/run_enrichment.py`, resumable + logfile + cost). **Two-pass, matching the audio-once/text-rerunnable split (#52):** (a) **capture-only** `--no-enrich` over all 517 banks transcripts+voiceprints once (the irreversible thermal one-shot, **$0 API**); (b) **enrich** reads those committed transcripts (no re-ASR) at any cadence. **Sonnet-only** (`claude-sonnet-4-6`, #53 — Haiku rejected on French/theological quality): **~$0.0705/sermon ⇒ ~$36 for the full 517**. Needs `ANTHROPIC_API_KEY`. EN/language + speaker now audio-truthful (#48–#52); YouTube retry/back-off in. *(Only audio-derived data — transcript/VTT/voiceprint — is one-shot; all enrichment is re-runnable from committed transcripts.)*
7. **JSON Schema + WP import** — freeze the canonical record contract; design the WordPress CPT/ACF import → first public deliverable (website sermon library).
