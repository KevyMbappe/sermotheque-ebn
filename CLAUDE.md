...
3. **Polish the web POC — it is LIVE** at **https://kevymbappe.github.io/sermotheque-ebn/** (#54/#55, M6c, 2026-08-01): 131 enriched sermons, player + chapter deep-links + synced transcript, mobile-clean 320→768 px, and it **republishes itself** whenever `data/catalog/**` changes. It remains a **demo for the elders + de-risking of the WordPress library**, not a replacement. La fiche sermon a été restructurée le 2026-08-02 (M6h) : ordre écoute-d'abord, blocs repliables, bloc lecteur collant. Left: the **YouTube IFrame path** (a distinct code path, never exercised — the dev container blocks YouTube), an EN sermon, the **8 sermons needing a ~$0.55 re-enrich** to be publishable, and **5 rows with an empty `title`** (upstream parser fix). Resume notes: `docs/plans/WEB-POC-STATUS.md`.
4. **Global transcript search** (deferred on purpose, 2026-08-01) — searching *inside* the 139 committed transcripts, not just the editorial fields. Needs a prebuilt static index (§8: no server); the current in-memory search would not scale to full text. Runs anywhere, no API.
5. **A proper design language for the POC** (roadmap, not urgent) — the current CSS is fluid but authored desktop-out (base styles + one `max-width: 820px` breakpoint), and the visual system grew page by page. Worth a deliberate pass before the site is shown widely, *not* a rewrite.
