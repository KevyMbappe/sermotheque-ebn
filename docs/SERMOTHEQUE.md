...
**M5g catalog correction — DONE (2026-06-14, decision #43).** The same run exposed a **false-positive YT↔SC match**: `IqNmh_XGULE` (Psaume 40) was wrongly attached to the Genèse SC sermon `sc-2054926060` via a 5-second duration collision (`youtube_match: 0.656`). Detached it and added the video as its own enriched entry `yt-IqNmh_XGULE` — **catalog 467 → 468**, `catalog.csv` regenerated. Flags a matcher-threshold audit (see #43).
...
- **#43 (2026-06-14) — YT↔SC matcher produces false positives at the duration-collision boundary (found in the wild):** Validating the pipeline on YouTube video `IqNmh_XGULE` ("Prier selon le cœur de Dieu IV | **Psaume 40**") surfaced that it had been auto-matched as the `youtube_id` of an *unrelated* SoundCloud sermon `sc-2054926060` ("Image de Dieu | **Genèse 1:26-2:3**") at `youtube_match: 0.656`. Cause: the duration fingerprint — YT 3555 s vs SC 3550 s, a **5-second collision between two different ~59-min sermons** — plus a weak title signal cleared the auto-accept bar. This is the concrete instance of the "dedup edge case" risk. **Immediate fix:** detached the bad match (`sc-2054926060.youtube_id → null`) and added the video as its own entry `yt-IqNmh_XGULE` (catalog now **468**). **Open / recommended:** (a) raise the auto-accept threshold and require corroborating title/scripture agreement when the duration delta is small; (b) **audit the accepted matches in the ~0.5–0.7 band** for other duration-collision false positives; (c) route the rest to a human review list rather than auto-linking. Until the matcher is hardened, the union's YT↔SC links in that confidence band are suspect. *(Note: hand-patches to `catalog.json` are reverted by re-running `build.py` — the durable fix lives in `match_youtube.py`.)*
