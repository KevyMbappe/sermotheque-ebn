...
**M5b POC — DONE (2026-06-14).** Ran the real pipeline on the 8 sample sermons (full audio, 7–11.5k words each): scripture/kind parsed correctly (incl. cross-chapter `Gen.1.1-Gen.2.3`, `Leçon`→teaching), Claude enrichment added (topics/summary/series — recovered series context titles lacked). 8 transcripts committed to `data/catalog/transcripts/`; demo entries `docs/spike-asr/poc_entries.json`; elder-facing `docs/spike-asr/POC.{md,pdf}` (5 pp). Environment is now project-local (`.venv` + `requirements.txt`, `cache/`, `.env`/`.env.example`) — no `/tmp`. **Open design note:** ASR backend is pluggable (mlx-whisper now / cloud Whisper API e.g. Groq in production) — pending decision.
